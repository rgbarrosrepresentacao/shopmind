...
import { 
  ...
  XCircle, Edit, Trash2, ArrowUpDown, ChevronLeft, 
  ChevronRight, Phone, MessageSquare, AlertCircle, ShoppingBag
} from "lucide-react";

interface FinanceiroReceberProps {
...
const CATEGORIAS_RECEITA = ["Venda", "Investimentos", "Outros"];

export function FinanceiroReceber({
  onEditClick,
  onNewClick,
  refreshTrigger,
  onRefreshNeeded,
}: FinanceiroReceberProps) {
  ...
  const handleBaixarTitulo = async (id: string) => {
    const confirm = window.confirm("Confirmar recebimento deste título?");
    if (!confirm) return;
    ...
    <div className="bg-card border border-border/80 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-border flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3.5 bg-slate-50/20">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={onNewClick}
            className="px-4 py-2.5 text-xs font-bold rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg shadow-emerald-600/10 flex items-center gap-2 cursor-pointer transition-all"
          >
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border bg-slate-50/50 text-muted-foreground">
              ...
              <th className="text-left px-4 py-3 font-bold w-24">Recebimento</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={9} className="text-center py-16 text-muted-foreground">
                  ...
                  <p className="font-bold">Nenhum título a receber encontrado</p>
                  <p className="text-[10px] text-muted-foreground/80 mt-0.5">Sem pendências de faturamento no filtro atual.</p>
                </td>
              </tr>
            ) : (
              transacoes.map(t => (
                <tr key={t.id} className="border-b border-border/50 hover:bg-slate-50/30 transition-colors">
                      ...
                      <div className="space-y-0.5">
                        ...
                        {t.cliente?.telefone && (
                          <div className="flex items-center gap-1">
                            <a 
                              href={`https://wa.me/55${t.cliente.telefone.replace(/\D/g, "")}`} 
                              target="_blank" 
                              rel="noopener noreferrer" 
                              className="text-[9px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5"
                            >
                              <Phone className="w-2.5 h-2.5" /> Cobrar WhatsApp
                            </a>
                          </div>
                        )}
                      </div>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {/* Editar (Apenas lançamentos manuais não recebidos/cancelados) */}
      ...
      {totalPages > 1 && (
        <div className="p-4 border-t border-border flex items-center justify-between bg-slate-50/10 flex-shrink-0">
          <p className="text-[10px] text-muted-foreground font-bold">{total} contas a receber no total</p>
          <div className="flex items-center gap-1.5">
            <button
